...
int main() {
	...
	if (my_rank==0){
		...
		ofstream mat_nonzero("/home/ambershek/HPC/lab4/matrix_nonzero.mtx");
		int row, col;
		double value;
		string header;
		//int index_matrix=0;
		...
		while (!mat.eof()) {
			mat >> row>>col>>value;
			if (row > 0 && col > 0 && value != 0 && mat_nonzero.is_open()) {
				//element new_element=element(row,col,value);
				mat_nonzero<<row<<" "<<col<<" "<<value<<"\n";
				//cout << row << " " << col << " " << value << endl;
			}
			mat.get();
			if (mat.peek() == EOF) break;
		}
		//cout<<"Generate non-zero matrix!"<<endl;
	...
	//Barrier 1
	//Now non-zero elements of matrix A are in file "matrix_nonzero"
	MPI_Barrier(comm);
	...
	ifstream mat_nonzero("/home/ambershek/HPC/lab4/matrix_nonzero.mtx");
	if (mat_nonzero.is_open()){
			int row,col;
			double value;
			for (int i=0; i<displs[my_rank]; i++){
				mat_nonzero>>row>>col>>value;
			}
			for (int i=0; i<local_n; i++){
				mat_nonzero>>row>>col>>value;
				element new_element=element(row,col,value);
				local_matrix[i]=new_element;
			}
	}
