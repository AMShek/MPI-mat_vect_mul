...
void Read_matrix(
   ...
   if (my_rank == 0) {//0号进程读取向量
	...
	//替换成从文件读取的
	FILE *fid =fopen("testmatrix.mtx","r");
	while (!feof(fid)){
		fscanf(fid, "%d", &i);
		i--;
		//printf("read i as %d\n",i);//test
		...
		j--;
		//printf("read j as %d\n",j);//test
		fscanf(fid, "%lf", &A[i*n+j]);	
		//printf("read element as %lf\n",A[i*n+j]);//test
	}
      
      MPI_Scatter(A, 1, block_col_mpi_t, local_A, m*local_n, MPI_DOUBLE,
            0, comm);
      free(A);
   } else {//除0号外的其他进程分配0号进程读到的向量
      Check_for_error(local_ok, "Read_matrix",
                  "Can't allocate temporary matrix", comm);
      MPI_Scatter(A, 1, block_col_mpi_t, local_A, m*local_n, MPI_DOUBLE,
            0, comm);
...
void Print_matrix(char title[], double local_A[], int m, int local_n, 
   ...
   if (my_rank == 0) {
      ...
      printf("Writing the matrix %s...\n", title);
	FILE* fid=("matrix.mtx","w");
	if (fid){
	      for (i = 0; i < m; i++) {
		 for (j = 0; j < n; j++)
		    fprintf(fid, "%d\t%d\t%.2f\n ", i, j,A[i*n+j]);
...
void Read_vector(
   ...
   if (my_rank == 0) {
      ...
      /*for (i = 0; i < n; i++)
         scanf("%lf", &vec[i]);*/
	 //替换成从文件读取的
	FILE *fid =fopen("testvector.mtx","r");
	while (!feof(fid)){
		fscanf(fid, "%d", &i);
		i--;
		//printf("read i as %d\n",i);//test
...
void Print_vector(
   ...
   if (my_rank == 0) {
      ...
      printf("\nWriting the vector %s...\n", title);
	FILE * fid=fopen("result.mtx","w");
	if (fid){
	      for (i = 0; i < n; i++)
		 fprintf(fid, "%d\t%f\n ", i+1,vec[i]);
	}
